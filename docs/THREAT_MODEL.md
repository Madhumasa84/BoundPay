...
- Serialized reservation prevents concurrent commitments in the tested single-instance SQLite threat model from exceeding a mode-specific daily budget.
...
- One application instance and one persistent SQLite volume are assumed. In-memory mock-provider state is process-local.
